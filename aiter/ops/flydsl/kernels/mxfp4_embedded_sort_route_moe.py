...
def compile_mxfp4_routed_embedded_shared_stage1(
    *,
    D_HIDDEN,
    D_INTER,
    NE,
    ...
    """Embedded-sort routed slots plus one grouped deterministic shared slot."""
    if BM != 16 or BN not in (128, 256) or BK != 256:
        raise ValueError(
            "routed-embedded/shared Stage 1 requires BM=16, "
            "BN in {128,256}, and BK=256"
        )
    if BN == 128 and interleave:
        raise ValueError(
            "routed-embedded/shared BN128 supports separated gate/up only"
        )
    if TOPK < 2:
        raise ValueError("routed-embedded/shared Stage 1 requires TOPK >= 2")
    ...
    name = (
        f"mxfp4_routed_emsort_shared_g1_h{D_HIDDEN}_i{D_INTER}"
    ...
    def stage1_kernel(
        ...
        if bx_i32 < routed_candidate_blocks:
            ...
            if tx_i32 < route0:
                ...
                prev_slot = (
                    tx_i32 - prev_token * fx.Int32(routed_topk)
                )
            ...
            if is_leader:
                if tx_i32 < num_routed:
                    ...
                    route_slot = (
                        tx_i32
                        - route_token * fx.Int32(routed_topk)
                    )
                    route_raw = (
                        route_token * fx.Int32(TOPK) + route_slot
                    )
                    route_expert = _global_i32_at(
                        arg_topk_ids, route_raw
                    )
                    if route_expert == expert:
                        slot = llvm.AtomicRMWOp(
                            llvm.AtomicBinOp.add,
                ...
                for pass_idx in range_constexpr(
                    ...
                    _gemm1_body(
                        ...
                        routed_use_nt,
                        i32_ntok,
                        routed_candidate_blocks + fx.Int32(1),
                        BM=BM,
                        BN=BN,
                        BK=BK,
                        inline_quant=True,
