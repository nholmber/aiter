...
def _get_stage1(
    ...
    from .kernels.mxfp4_token_serial_shared_moe import (
        compile_mxfp4_token_serial_shared_stage1,
    )

    return compile_mxfp4_token_serial_shared_stage1(
        D_HIDDEN=D_HIDDEN,
        D_INTER=D_INTER,
        NE=NE,
        TOPK=TOPK,
...
def flydsl_mxfp4_token_serial_shared_moe(
    *,
    hidden_states,
    w1,
    w1_scale,
    ...
    """Experimental two-launch token/phase-dispatched GLM-5.2 MoE."""
    ...
    if M < 1 or M > BM:
        raise ValueError(
            f"token-serial path supports 1 <= M <= {BM}, got {M}"
        )
    if TOPK != 9:
        raise ValueError(
            "token-serial path requires eight routed slots plus one shared slot"
        )
    if D_HIDDEN % 256 != 0 or D_INTER % 256 != 0:
        raise ValueError(
            "token-serial path requires hidden/inter dimensions divisible by 256"
        )
